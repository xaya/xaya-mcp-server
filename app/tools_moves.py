...
class Tools:
  ...
  def __init__ (self, operator_account=None, contract_tools=None):
    ...
    # Track the last used nonce per address locally.
    # We use max(pending_chain_nonce, local_nonce + 1) to handle both
    # rapid local submissions (where chain might lag) and external transactions.
  ...
  async def sendMove (self, ns, name, move, privkey=None, gas_gwei=None):
    ...
    chain_nonce = await w3.eth.get_transaction_count(acc.address, 'pending')
    local_last_nonce = self.nonces.get(acc.address, -1)
    nonce = max(chain_nonce, local_last_nonce + 1)
    self.nonces[acc.address] = nonce
    ...
    gas_estimate_transaction = await tx.build_transaction(tx_dict)
    tx_dict["gas"] = await w3.eth.estimate_gas(gas_estimate_transaction)
    signed_tx = w3.eth.account.sign_transaction(await tx.build_transaction(tx_dict),
                                                acc.key)
    tx_hash = await w3.eth.send_raw_transaction(signed_tx.raw_transaction)
    return {"txid": "0x" + tx_hash.hex()}

  def _validate_txids(self, txids: Union[str, List[str]]) -> List[str]:
    """
    Validates and normalizes transaction IDs.
    
    Args:
      txids: Single transaction ID as hex string or list of transaction IDs
      
    Returns:
      List of validated transaction IDs
    """
  ...
  async def getTransactionStatus (self, txids: Union[str, List[str]]) -> Dict[str, str]:
    """
    Gets the current status of one or more transactions without waiting.

    Args:
      txids: Single transaction ID as hex string or list of transaction IDs

    Returns:
      Dictionary with txid as keys and status as values (pending, success, reverted, notfound)
    """
    txid_list = self._validate_txids(txids)
    ...
    for txid in txid_list:
      try:
        # Check transaction details first (to catch pending state)
        tx = await w3.eth.get_transaction(txid)

        if tx.blockNumber is None:
          result[txid] = "pending"
        else:
          # Transaction is mined, get receipt for final status
  ...
  async def waitForTransactions (self, txids: Union[str, List[str]]) -> Dict[str, str]:
    """
    Waits for one or more transactions to be mined and returns their status.

    Args:
      txids: Single transaction ID as hex string or list of transaction IDs

    Returns:
      Dictionary with txid as keys and status as values (success, reverted, notfound)
    """
    txid_list = self._validate_txids(txids)
